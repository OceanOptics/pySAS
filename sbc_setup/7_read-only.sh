...
echo "Read-Only"
echo "========="
echo "Source: https://learn.adafruit.com/read-only-raspberry-pi"
...
read -r -d '' FOO <<- 'EOM' 
	...
	echo -n "Enable boot-time read/write jumper (pySAS=Y)? [y/N] "
	read
	if [[ "$REPLY" =~ (yes|y|Y)$ ]]; then
		INSTALL_RW_JUMPER=1
		echo -n "GPIO pin for R/W jumper (pySAS=21): "
		read
		RW_PIN=$REPLY
	fi

	echo -n "Install GPIO-halt utility (pySAS=N)? [y/N] "
	read
	if [[ "$REPLY" =~ (yes|y|Y)$ ]]; then
		INSTALL_HALT=1
	...
	echo -n "Enable kernel panic watchdog (pySAS=N)? [y/N] "
	read
	if [[ "$REPLY" =~ (yes|y|Y)$ ]]; then
		INSTALL_WATCHDOG=1
	...
	# pySAS custom folders to move
	rm -rf /var/lib/misc
	ln -s /tmp /var/lib/misc
	# Source for following: https://k3a.me/how-to-make-raspberrypi-truly-read-only-reliable-and-trouble-free/
